...
And so I left this topic for some time.

About half a year later I heard a podcast episode on small language models. Guests discussed tiny models for edge devices, e.g.
a standing desk that knows when I need a stretch and raises its height, or lighting that adapts to light conditions. The conversation went on to
discuss other topics like privacy and automation. An interesting topic was a discussion of physical materials: a custom-trained model for
...
The moment I finally started looking into local models was when Mistral released their Shieldstral model. It is a specialized model for content moderation.
"Shieldstral frames content moderation as a binary question-answering task" ([source](https://mistral.ai/news/shieldstral/)). In simple words,
the model receives input and responds with yes/no and a score. For example, does this text promote violence? Does this image contain drugs?
Finally, I knew how this could be applied to my daily work, and built a tool around this model. This work sent me on an interesting journey that
I will describe in a series of posts here.
...
with such a list it feels very frightening, at least to me. What are all these abbreviations? What should I choose if I want to have the same experience
...
Let's start with the most important part here, the parameter count. Usually this is the section that immediately follows the model name, e.g. `qwen3.8-2.4T`
or `qwen3.8-27B`. In the first, the model has 2.4 trillion parameters; in the latter, only 27 billion. Knowing number of parameters
...
Oftentimes you will see something like `A3B`, a letter "A" followed by the number of parameters. For example, a Google's Gemma 4 model `gemma-4-26B-A4B`
...
model which uses a sort of router to activate only 4B parameters for each token. This brings compute savings over a dense model where all 26B params
...
"currently-needed" experts from disk or network which allows you to use 1-2.8T parameter models on computers where the model cannot fit into the VRAM. Some
people managed to run Kimi K3, a 2.8T model, on a Macbook M1 Max and stream the model from four SSDs at about 15 tokens per minute. [source](https://github.com/gavamedia/deltafin)
...
lose significant precision. FP is not the only option you can see. For integer precision, you could see `Q4_K_M`, `INT8`, `Q8_0` which are all integer-based.
...
Let's look at the different types of the Gemma 4 model: `A4B`, `A4B-it`, `A4B-it-assistant`. The first one is the so called base model. This is the raw
...
fine-tune for your own needs. It can understand text, so you might fine-tune it to spot patterns like a vulnerable network response.
...
what the next handful of tokens a full model would likely generate. The job of the "big" model is then to verify whether the predicted tokens are correct or
not. When the prediction is correct, you can get up to a 2x boost in the token generation speed.
...
`mlx-community/Qwen3.8-27B-4bit`. MLX is a machine learning framework built by Apple for its chips. You generally want to run models from the
...
This should be enough information to decode most of the names. In the next post I'll dive into how to run these models and what it costs.
